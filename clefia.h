#define a0_8   0xff000000
#define a8_16  0x00ff0000
#define a16_24 0x0000ff00
#define a24_32 0x000000ff
#define BLOCK_TYPE unsigned int
#define BLOCK_SIZE 16
#define WORD_FROM_BYTES(_a,_b,_c,_d) (unsigned int)(((_a << 24) | 0x00ffffff) & ((_b << 16) | 0xff00ffff ) & ((_c << 8) | 0xffff00ff) & (_d | 0xffffff00));
#define ROUNDS 18

BLOCK_TYPE S0[256] = {};

BLOCK_TYPE S1[256] = {};

BLOCK_TYPE RoundKeys[ROUNDS * 2] = {0x123245ba, 0x123245ba, 
                                    0x123245ba, 0x123245ba, 
                                    0x123245ba, 0x123245ba,
                                    0x123245ba, 0x123245ba, 
                                    0x123245ba, 0x123245ba, 
                                    0x123245ba, 0x123245ba, 
                                    0x123245ba, 0x123245de,
                                    0x123245ba, 0x123245de,
                                    0x123245ab, 0x123245ba,
                                    0x123245ba, 0x123245ba, 
                                    0x123245ba, 0x123245ba, 
                                    0x123245ba, 0x123245ba,
                                    0x123245ba, 0x123245ba, 
                                    0x123245ba, 0x123245ba, 
                                    0x123245ba, 0x123245ba, 
                                    0x123245ba, 0x123245de,
                                    0x123245ba, 0x123245de,
                                    0x123245ab, 0x123245ba};

BLOCK_TYPE WhiteKeys[4] = {0xab345601, 0x39328382, 0x93232329, 0x34084756};